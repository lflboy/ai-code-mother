package cn.longwingstech.intelligence.longaicodemother.monitor;
...
public class AiModelMonitorListener implements ChatModelListener {
    ...
    public void onRequest(ChatModelRequestContext requestContext) {
        log.info("onRequest当前线程为：{}",Thread.currentThread().getName());
        // 获取当前时间戳，但未做任何处理
    ...
    public void onResponse(ChatModelResponseContext responseContext) {
        ...
        // 记录 Token 使用情况(无userId和appid版)
        recordTokenUsage(responseContext,modelName);
        log.info("modelName:{}，tokens：{}",modelName,responseContext.chatResponse().tokenUsage().totalTokenCount());
        if(userId != null) {
            log.info("请求用户：{}",userId);
            log.info("应用appId:{}",appId);
            // 记录成功请求
    ...
    private void recordTokenUsage(ChatModelResponseContext responseContext, String userId, String appId, String modelName) {
        ...
        if (tokenUsage != null) {
            ...
            log.info("input tokens：{}",tokenUsage.inputTokenCount());
            log.info("output tokens：{}",tokenUsage.outputTokenCount());
            log.info("total tokens：{}",tokenUsage.totalTokenCount());
        }
    }
    /**
     * 记录Token使用情况(无userId和appId)
     */
    private void recordTokenUsage(ChatModelResponseContext responseContext, String modelName) {
        TokenUsage tokenUsage = responseContext.chatResponse().metadata().tokenUsage();
        if (tokenUsage != null) {
