...
public class AiModelMetricsCollector {
    ...
    private final ConcurrentMap<String, Counter> allTokenCountersCache = new ConcurrentHashMap<>();
    ...
    public void recordTokenUsage( String modelName,
                                 String tokenType, long tokenCount) {
        String key = String.format("%s_%s", modelName, tokenType);
        Counter counter = allTokenCountersCache.computeIfAbsent(key, k ->
                Counter.builder("model_tokens_total")
                        .description("AI模型Token消耗总数(没有用户信息版本)")
                        .tag("model_name", modelName)
                        .tag("token_type", tokenType)
                        .register(meterRegistry)
        );
